...
class PlayState extends MusicBeatState {
	...
	var dirToVowel:Array<String> = ['a', 'e', 'i', 'o', 'u'];
	var vowelSounds:Map<String, Map<String, FlxSound>> = new Map();

	private static var _lastLoadedModDirectory:String = '';
	public static var nextReloadAll:Bool = false;

	function loadCharacterVoices(charName:String) {
		var charMap:Map<String, FlxSound> = new Map();
		for (vowel in dirToVowel) {
			var soundPath = Paths.sound('voices/' + charName + '/' + vowel);
			var sound = FlxG.sound.load(soundPath, 1, false);
			charMap.set(vowel, sound);
		}
		vowelSounds.set(charName, charMap);
	}
	...
	override public function create() {
		...
		loadCharacterVoices(boyfriend.curCharacter);
		loadCharacterVoices(dad.curCharacter);
	...
	function opponentNoteHit(note:Note):Void {
		var vowel = dirToVowel[note.noteData];
		if (vowelSounds.exists(dad.curCharacter)) {
			var charMap = vowelSounds.get(dad.curCharacter);
			if (charMap.exists(vowel)) {
				charMap.get(vowel).play(true);
			}
		}
	...
	public function goodNoteHit(note:Note):Void {
		var vowel = dirToVowel[note.noteData];
		if (vowelSounds.exists(boyfriend.curCharacter)) {
			var charMap = vowelSounds.get(boyfriend.curCharacter);
			if (charMap.exists(vowel)) {
				charMap.get(vowel).play(true);
			}
		}
